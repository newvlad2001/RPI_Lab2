...
function initTipsAndDotsSection() {
    ...
    for (let i = 0; i < tipsArray.length; i++) {
        tipsInfo.insertAdjacentHTML('afterbegin', '<div class="tips__tip-data fade">' + tipsArray[i] + '</div>');
        dotsSection.insertAdjacentHTML('afterbegin', '<span class="dot" onclick="setTip(' + (tipsArray.length - 1 - i) + ')"></span>');
    }
